...
def trm_cauchy(f, x0, delta0, delta_max, eta, iter_time, tolerance):
    ...
    for k in range(iter_time):
        p = cauchy(f, x, delta)
        ar = f(x) - f(x + p)
        pr = -f(x) + (-f(x) + np.dot(np.gradient(f, x), p))
        rho = ar / pr
        ...
        if np.linalg.norm(np.gradient(f, x)) < tolerance:
            break
...
def subspace(f, x, delta):
    ...
    elif np.all(np.linalg.eigvals(B) >= 0) and np.any(np.linalg.eigvals(B) == 0):
...
def trm_subspace(f, x0, delta0, delta_max, eta, iter_time, tolerance):
    ...
    for k in range(iter_time):
        ...
        ar = f(x) - f(x + p)
        pr = -f(x) + (-f(x) + np.dot(grad_f(x), p)
                      + 0.5 * np.dot(p, np.dot(hess_f(x), p)))
        rho = ar / pr
